...
def get_cached_api_response(path: str, query: str = ''):
...
def invalidate_product_cache(product_id, slug):
    ...
    L3.delete(_api_response_key('/api/books/', ' '))
    L3.delete(_api_response_key(f'api/books/{slug}', ' '))
    logger.info(f'[CACHE INVALIDATE] product_id={product_id}, slug={slug}')
